...
-(AppData*)init {
  ...
  if (self) {
    ...
    // Why is there this coordinate?
    // self.currentCoordinate = self.coordinate = CLLocationCoordinate2DInvalid();
    ...
    self.groundZeroCoordinate = CLLocationCoordinate2DMake(SENDAI_AIRPORT_LATITUDE, SENDAI_AIRPORT_LONGITUDE);
    //        self.groundZeroCoordinate = CLLocationCoordinate2DMake(GROUND_ZERO_LATITUDE, GROUND_ZERO_LONGITUDE);
    self.groundZeroRegion = MKCoordinateRegionMake(self.groundZeroCoordinate, MKCoordinateSpanMake(GROUND_ZERO_SPAN, GROUND_ZERO_SPAN ));
    ...
    if ([self.locationManager respondsToSelector:@selector(requestWhenInUseAuthorization)]) {
      // iOS8以降は、ユーザーにリクエストできる
      [self.locationManager requestWhenInUseAuthorization];
    } else {
      // TODO: アラート表示: 位置情報サービスを有効にして下さい
    }
  }
  return self;
}
...
- (void)getAndParseInfomation {
  ...
  for( Article* article in self.articles) {
    ...
    article.screenOffset = 1 * (step * kARViewInfoTagEstimateHeight) +  (arc4random() % 10);
              ...
              imageEnabled:(BOOL)imageEnabled
{
  ...
  for( KMLPlacemarkAnnotation* annotation in kmlPlacemarkPoints) {
    ...
    // 仮想爆心を試すときは「_VIRTUAL_COORDINATE」を消す
...
//               imageEnabled:YES];
  ...
  //    [self getAndParseInfomation]; //距離計算を行い、表示するかどうかを決める
