...
- (NSString *)sendUpdate:(NSString *)status latitude:(double)latitude longitude:(double)longitude
{
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:0];
    ...
    if (latitude >= -90.0 && latitude <= 90.0 && longitude >= -180.0 && longitude <= 180.0) {
        ...
        return [self sendUpdate:status params:params];
    }
    else {
        return [self sendUpdate:status params:nil];        
    }
...
- (NSString *)sendUpdate:(NSString *)status params:(NSMutableDictionary*)params
{
    if (!status) {
        return nil;
    }
